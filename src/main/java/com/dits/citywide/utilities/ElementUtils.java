...
//	}
	...
	// Helper methods for dropdown WebElements
	...
	public void selectFromReactDropdown(By dropdownLocator, String optionText, int timeout) {
	    WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(timeout));
	    JavascriptExecutor js = (JavascriptExecutor) driver;
	    Actions actions = new Actions(driver);

	    try {
	        // 1. Wait for and scroll dropdown into view
	        WebElement dropdown = wait.until(ExpectedConditions.elementToBeClickable(dropdownLocator));
	        js.executeScript("arguments[0].scrollIntoView({block: 'center'});", dropdown);

	        // 2. Click to activate the dropdown input
	        actions.moveToElement(dropdown).click().perform();
	        Thread.sleep(300); // brief delay to let dropdown open

	        // 3. Type the desired option (case-insensitive match)
	        actions.sendKeys(optionText).pause(Duration.ofMillis(500))
	               .sendKeys(Keys.ENTER)
	               .build()
	               .perform();

	        // 4. Optional: wait for dropdown to close (ensures selection applied)
	        wait.until(ExpectedConditions.invisibilityOfElementLocated(
	            By.xpath("//div[contains(@class,'ant-select-dropdown') and not(contains(@style,'display: none'))]")
	        ));

	        System.out.println("✅ Selected option using Actions: " + optionText);

	    } catch (Exception e) {
	        throw new RuntimeException("❌ Failed to select option via Actions: " + optionText, e);
	    }
	}
