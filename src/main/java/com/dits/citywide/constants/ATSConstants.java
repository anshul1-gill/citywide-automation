package com.dits.citywide.constants;

public class ATSConstants {

    // Settings - Job Post Form
    public static final String JOB_FORM_NAME = "Correctional Officer";

    // Settings - Source Type
    public static final String SOURCE_TYPE_NAME = "Referral";

    // Settings - Skill/Qualification
    public static final String SKILL_QUALIFICATION_NAME = "General Educational Development";

    // Job Post Form
    public static final String JOB_TITLE = "Dispatch Post";
    public static final String JOB_DEPARTMENT = "HR";
    public static final String JOB_LOCATION = "Riverside";
    public static final String JOB_EMPLOYMENT_TYPE = "Full time shift";
    public static final String JOB_END_DATE = "10/26/2025";
    public static final String JOB_QUALIFICATIONS = "General Educational Development";
    public static final String JOB_EXPERIENCE = "2 years";
    public static final String JOB_OPENINGS = "5";
    public static final String JOB_SALARY_RANGE = "40000 - 60000";
    public static final String JOB_STATUS = "Published";
    public static final String JOB_MODE = "Remote";
    public static final String JOB_FORM_SELECT = "Security Guard";
    public static final String JOB_DESCRIPTION = "Hiring for security guard position";
    public static final String JOB_TITLE_SEARCH = "Dispatch Post";
    
    // Application Form / Candidate Info
    public static final String CANDIDATE_FIRSTNAME = "Guss";
    public static final String CANDIDATE_LASTNAME = "Atinkson";
    public static final String CANDIDATE_EMAIL = "mathew.doe@example.com";
    public static final String CANDIDATE_PHONE = "1234567890";
    public static final String CANDIDATE_POSITIONS = "Dispatch Post";
    public static final String CANDIDATE_SOURCE = "Referral";
    public static final String CANDIDATE_SKILLS = "MCA";
    public static final String CANDIDATE_EMPLOYEE = "Dallas Police Department (DPD)";
    public static final String CANDIDATE_ROLE = "Deputy Chief";
    public static final String CANDIDATE_STATUS ="Sourced";
    public static final String CANDIDATE_RESUME_FILE_PATH = "/home/ditsdev151/Downloads/nm06zk9948_Sample certs.docx.pdf";

    // Interview
    public static final String INTERVIEW_DATE = "10/01/2025";
    public static final String TIME_SLOT = "45 Minutes";
    public static final String INTERVIEW_START = "08:30";
    public static final String INTERVIEW_MODE = "Walk In";
    public static final String INTERVIEW_ADDRESS ="D-210 Ditstek";
    public static final String INTERVIEWER_NAME ="Jack Smith";
    public static final String INTERVIEWER_NOTES = "The candidate has cleared the Interview.";

    // Offer
    public static final String OFFER_EXPIRE_DATE = "10/30/2025";
    public static final String OFFER_SUBJECT = "DISPATCH POSITION";
    public static final String OFFER_NOTES = "Hi Candidate , You have successfully cleared the interview";
    
    public static final String REJECTION_REASON = "Candidate did not meet  criteria.";
    public static final String BLACKLISTED_REASON ="Bad candidate ";
    
   }


